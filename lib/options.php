...
class Options
{
    ...
    public function __construct($module_id, $aSitesTabs,  $arTabs, $arGroups, $arOptions, $need_access_tab = false)
    {
        ...
        if($need_access_tab)
            $this->arTabs[] = array(
                'DIV' => 'edit_access_tab',
                'TAB' => Loc::getMessage('MEDIALINE_EASYFORM_EDIT_ACCESS_TAB'),
                'ICON' => '',
                'TITLE' => Loc::getMessage('MEDIALINE_EASYFORM_EDIT_ACCESS_TITLE'),
            );
    ...
    public function ShowHTML()
    {
        ...
        if(is_array($this->arOptions))
        {
                ...
                foreach ($arOptParamsSite as $option => $arOptParams) {
                    ...
                    switch ($arOptParams['TYPE']) {
                        ...
                        default:
                            ...
                            $input = '<input type="' . ($arOptParams['TYPE'] == 'INT' ? 'number' : 'text') . '" size="' . $arOptParams['SIZE'] . '" maxlength="' . $arOptParams['MAXLENGTH'] . '" value="' . htmlspecialchars($arOptParams['DEFAULT']) . '" name="' . htmlspecialchars($opt) . '" />';
                            if ($arOptParams['REFRESH'] == 'Y')
                                $input .= '<input type="submit" name="refresh" value="OK" />';
                            break;
                    }
                    ...
                    if (isset($arOptParams['NOTES']) && $arOptParams['NOTES'] != '')
                        $input .= '<div>
                                        <table cellspacing="0" cellpadding="0" border="0" class="notes" style="margin: 0 auto;">
                                            <tbody>
                                                <tr class="top">
            ...
            echo     '<input type="hidden" name="update" value="Y" />
                                      <input type="submit" name="save" value="'.Loc::getMessage('MEDIALINE_EASYFORM_SAVE').'" />
                                      <input type="reset" name="reset" value="'.Loc::getMessage('MEDIALINE_EASYFORM_RESET').'" />';
